...
<html lang="en" dir="ltr">
	<head>
		<meta charset="UTF-8">
		<title><?= $application_name; ?> | Orders List</title>
			...
			<div class="content-wrapper">
				...
				<div class="content-header">
					<div class="container-fluid">
						<div class="row mb-2">
							<div class="col-sm-6">
								<h1 class="m-0 text-dark"><small>Orders List</small></h1>
							</div>
							<!-- /.col -->
							<div class="col-sm-6">
					...
					<div class="card">
						<div class="card-header">
							<ul class="nav nav-tabs align-items-end card-header-tabs w-100">
								<li class="nav-item">
									<a class="nav-link active" href="#"><i class="fa fa-list mr-2"></i>Items List</a>
								</li>
							</ul>
						</div>
						<div class="card-body">
							...
							<table id="example2" class="table table-bordered table-hover">
                               ...
                               if (!empty($orders)) {
                                    foreach ($orders as $item) {
                                      ...
                                      echo "<tr><td>{$item['user_mobile']}</td><td>{$item['dateCreated']}</td><td>{$item['reservationCost']}</td><td>{$item['reservations']}</td><td>{$item['status']}</td><td>{$item['payMode']}</td><td>{$item['total']}</td><td>{$item['bookingDate']}</td><td><form method=\"POST\" action=\"delete-order.php?id={$item['id']}\" accept-charset=\"UTF-8\"><button type=\"submit\" class=\"btn btn-link text-danger\" onclick=\"return confirm('Are you sure?')\"><i class=\"fa fa-trash\"></i></button>
												</form></td></tr>";
                                    }
                               }
                               ?>
