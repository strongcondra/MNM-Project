<?php
   ...
   function get_orders() {
      if (authenticated()) {
         $statement = get_connection()->prepare("SELECT `id`, `total`, `foodTotal`, `status`, `reservationCost`, `dateCreated`, `reservations`, `payMode`, `user_mobile`, `placeName`,`placeImage` FROM `orders`");
         $statement->execute();
