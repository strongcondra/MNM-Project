        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">List of Cafés</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                     ...
                     <div class="row">
                        <div class="col-sm-12">
                          <table id="example2" class="table table-bordered table-hover dataTable dtr-inline" role="grid" aria-describedby="example2_info">
                              <thead>
                                 <tr role="row">
                                    <th class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending">Owner Name</th>
                                    <th class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending">Mobile</th>
                              ...
                              <tbody>
                               ...
                               if (!empty($cafe)) {
                                    foreach ($cafe as $item) {
                                      echo "<tr><td>{$item['owner_name']}</td><td>{$item['owner_mob']}</td><td>{$item['cafe_name']}</td><td>{$item['location']}</td><td>{$item['information']}</td><td>" . (empty($item['status'])  ? "Not Active" : "Active") . "</td><td>{$item['cuisine']}</td><td>{$item['description']}</td><td> <a href=\"delete-cafe.php?id={$item['cafe_id']}\" title=\"Delete\"class=\"btn btn-danger\">Delete</a>&nbsp &nbsp  <a href=\"accept-cafe.php?id={$item['cafe_id']}\" class=\"btn btn-success\" title=\"Accept\">Accept</a></td></tr>";
                                    }
                               }
                               ?>
                              </tbody>
