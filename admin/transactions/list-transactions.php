...
<?php
    ...
    function get_transactions(){
        if(authenticated()){
			$statement = get_connection()->prepare("SELECT `transactions`.`transaction_id`,`transactions`.`amount`,`users`.`user_name`,`image_list`.`cafe_name`, `transactions`.`date` FROM `transactions` INNER JOIN `users` ON `transactions`.`user_id`=`users`.`user_id` INNER JOIN `image_list` ON `transactions`.`cafe_id`= `image_list`.`image_id` ");
			$statement->execute();
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">List of Transactions</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                              ...
                              <tbody>
                               ...
                               if (!empty($transactions)) {
                                    foreach ($transactions as $item) {
                                      echo "<tr><td>{$item['transaction_id']}</td><td>{$item['user_name']}</td><td>{$item['cafe_name']}</td><td>{$item['amount']}</td><td>{$item['date']}</td><td><a href=\"delete-transaction.php?id={$item['transaction_id']}\" title=\"Delete\"class=\"btn btn-danger\">Delete</a></tr>";
                                    }
                               }
                               ?>
                              </tbody>
