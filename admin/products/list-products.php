...
<?php
    ...
    function list_products(){
        if(authenticated()){
			$statement = get_connection()->prepare("SELECT `products`.`product_id`,`image_list`.`cafe_name`,`products`.`product_name`,`products`.`product_price`,`products`.`product_quant`FROM `products` INNER JOIN `image_list` ON `products`.`cafe_id`=`image_list`.`image_id` ");
			$statement->execute();
                              ...
                              <tbody>
                               ...
                               if (!empty($products)) {
                                    foreach ($products as $item) {
                                      echo "<tr><td>{$item['product_id']}</td><td>{$item['cafe_name']}</td><td>{$item['product_name']}</td><td>{$item['product_price']}</td><td>{$item['product_quant']}</td><td><a href=\"delete-product.php?product_id={$item['product_id']}\" title=\"Delete\"class=\"btn btn-danger\">Delete</a> </td></tr>";
                                    }
                               }
                               ?>
                              </tbody>
