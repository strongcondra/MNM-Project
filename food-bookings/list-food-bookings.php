<?php
    ...
    $food_bookings = get_food_bookings();
...
<html lang="en" dir="ltr">
	<head>
		<meta charset="UTF-8">
		<title><?= $application_name; ?> | Food Bookings List</title>
			...
			<div class="content-wrapper">
				...
				<div class="content-header">
					<div class="container-fluid">
						<div class="row mb-2">
							<div class="col-sm-6">
								<h1 class="m-0 text-dark"><small>Food Bookings List</small></h1>
							</div>
							<!-- /.col -->
							<div class="col-sm-6">
								<ol class="breadcrumb float-sm-right">
									<li class="breadcrumb-item"><a href="<?= $base_url; ?>dashboard.php"><i class="fa fa-dashboard"></i> Dashboard</a></li>
									<li class="breadcrumb-item active">Food Bookings List</li>
								</ol>
							</div>
							<!-- /.col -->
						</div>
					...
					<div class="card">
						<div class="card-header">
							<ul class="nav nav-tabs align-items-end card-header-tabs w-100">
								<li class="nav-item">
									<a class="nav-link active" href="<?= $base_url; ?>list-records.php"><i class="fa fa-list mr-2"></i>Food Bookings List</a>
								</li>
							</ul>
						</div>
						<div class="card-body">
							...
							<table id="example2" class="table table-bordered table-hover">
				                ...
				                <tbody>
				                	...
				                	<?php
                               if (!empty($food_bookings)) {
                                    foreach ($food_bookings as $item) {
                                      echo "<tr><td>{$item['placeName']}</td><td>{$item['cost']}</td><td>{$item['foodName']}</td><td>{$item['userMobile']}</td><td>{$item['reservations']}</td><td>{$item['foodDescription']}</td><td><form method=\"POST\" action=\"delete-food-booking.php?id={$item['id']}\" accept-charset=\"UTF-8\"><button type=\"submit\" class=\"btn btn-link text-danger\" onclick=\"return confirm('Are you sure?')\"><i class=\"fa fa-trash\"></i></button>
												</form></td></tr>";
                                    }
                               }
                               ?>
