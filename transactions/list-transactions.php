...
<html lang="en" dir="ltr">
	<head>
		<meta charset="UTF-8">
		<title><?= $application_name; ?> | Transactions List</title>
			...
			<div class="content-wrapper">
				...
				<div class="content-header">
					<div class="container-fluid">
						<div class="row mb-2">
							<div class="col-sm-6">
								<h1 class="m-0 text-dark"><small>Transactions List</small></h1>
							</div>
							<!-- /.col -->
							<div class="col-sm-6">
					...
					<div class="card">
						<div class="card-header">
							<ul class="nav nav-tabs align-items-end card-header-tabs w-100">
								<li class="nav-item">
									<a class="nav-link active" href="#"><i class="fa fa-list mr-2"></i>Transactions List</a>
								</li>
							</ul>
						</div>
						<div class="card-body">
							...
							<table id="example2" class="table table-bordered table-hover">
                               ...
                               if (!empty($transactions)) {
                                    foreach ($transactions as $item) {
                                      echo "<tr><td>{$item['user_name']}</td><td>{$item['cafe_name']}</td><td>{$item['amount']}</td><td>{$item['date']}</td><td><form method=\"POST\" action=\"delete-transaction.php?id={$item['transaction_id']}\" accept-charset=\"UTF-8\"><button type=\"submit\" class=\"btn btn-link text-danger\" onclick=\"return confirm('Are you sure?')\"><i class=\"fa fa-trash\"></i></button>
												</form></td></tr>";
                                    }
                               }
                               ?>
