<?php
    ...
    function get_facility_value($facility_id){
    if (isset($facility_id) ) {
        
        $statement = get_connection()->prepare("SELECT `id`, `facility`,`price` FROM `additionalFacility` WHERE `id`=? AND `owner_mob`=?");
        $statement->execute([$facility_id,$_SESSION['owner_mob']]);
	...
	function update_additional_facility($id,$facility,$price){
	    if(!empty($id) && !empty($facility)&& !empty($price)){
	        $statement = get_connection()->prepare("UPDATE  `additionalFacility` SET `facility`=?,`price`=? WHERE  `id`=?");
	        $statement->execute([$facility,$price,$id]);
	...
	if(isset($_POST['facility_id']) && isset($_POST['facility']) && isset($_POST['price'])){
	    $facility_id = (int) $_POST['facility_id'];
	    $facility = $_POST['facility'];
	    $price = $_POST['price'];
	    
	    if(!empty($facility_id) && !empty($facility) && !empty($price) ){
	        if(update_additional_facility($facility_id,$facility,$price)){
	...
	$rows = get_facility_value($facility_id);
	$id = $rows['id'];
...
<html lang="en" dir="ltr">
	<head>
		<meta charset="UTF-8">
		<title><?= $application_name; ?> | Edit Facility</title>
			...
			<div class="content-wrapper">
				...
				<div class="content-header">
					<div class="container-fluid">
						<div class="row mb-2">
							<div class="col-sm-6">
								<h1 class="m-0 text-dark"><small>Edit Facility</small></h1>
							</div>
							<!-- /.col -->
							<div class="col-sm-6">
					...
					<div class="card">
						<div class="card-header">
							<ul class="nav nav-tabs align-items-end card-header-tabs w-100">
								<li class="nav-item">
									<a class="nav-link active" href="#"><i class="fa fa-list mr-2"></i>Edit Facility</a>
								</li>
							</ul>
						</div>
					            ...
					            <div class="row">
					            	<div class="col-md-7">
						            	...
						            	<div class="form-group">
							        		...
							        		<input type="text" name="facility" class="form-control" id="facility" value="<?=$facility;?>">
							        	</div>
						            	<div class="form-group">
							        		<label for="email">Price</label>
							        		<input type="text" name="price" class="form-control" id="price" value="<?=$price;?>">
							        	</div>
							        	<input type="hidden" name="facility_id"  class="form-control" id="facility_id" value=" <?=$id;?>" >
