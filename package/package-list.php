...
$statement = $mysqli->query("SELECT *  FROM package_details where cafe_id = ".$cafeId);
			...
			<div class="content">
				...
				<div class="card">
					...
					<div class="card-body">
                           <table id="example2" class="table table-bordered table-hover dataTable dtr-inline" role="grid" aria-describedby="example2_info">
                              <thead>
                                 <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Details</th>
                                    <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Starter</th>
                                    <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Mains</th>
                                    <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Desserts</th>
                              ...
                              <tbody>
                               ...
                               if (!empty($package)) {
                                    foreach ($package as $item) {
                                      ...
                                      echo "<tr><td>{$item['id']}</td><td>{$item['name']}</td><td>{$item['type']}</td><td>{$item['details']}</td><td>{$item['starter']}</td><td>{$item['mains']}</td><td>{$item['desserts']}</td><td><div class='custom-control custom-checkbox'>
                        ...
                        " . ($item['status']=='1'  ? "Active" : "Non-Active") . "</label></div></td><td><a href=\"delete-package.php?id={$item['id']}\"class=\"btn btn-danger\"  title=\"Delete\">Delete</a> &nbsp &nbsp <a href=\"{$base_url}package/edit-package.php?id={$item['id']}\" class=\"btn btn-success\" title=\"Edit\">Edit</a></td></tr>";
                                    }
                               }
                               ?>
                              </tbody>
